    ...
    public class EarningsEventProfile : Profile
    {
        public EarningsEventProfile()
        {
            CreateMap<IntermediateLearningAim, EarningEvent>()
                ...
                .AfterMap((intermediateLearningAim, earningEvent) => earningEvent.PriceEpisodes.ForEach(pe =>
                    pe.CourseStartDate = intermediateLearningAim.Aims
                        ...
                        .LearnStartDate))
                .AfterMap((intermediateLearningAim, earningEvent) => earningEvent.LearningAim.FundingLineType = earningEvent.PriceEpisodes.FirstOrDefault(p => p.LearningAimSequenceNumber == earningEvent.LearningAim.SequenceNumber)?.FundingLineType)
                ;
            ...
            CreateMap<IntermediateLearningAim, LearningAim>()
                ...
                .ForMember(x => x.FundingLineType, opt => opt.Ignore())
            ...
            CreateMap<ESFA.DC.ILR.FundingService.FM36.FundingOutput.Model.Output.PriceEpisode, Payments.Model.Core.PriceEpisode>()
                ...
                .ForMember(dest => dest.LearningAimSequenceNumber, opt => opt.MapFrom(source => source.PriceEpisodeValues.PriceEpisodeAimSeqNumber))
                ;
        }
    }
}